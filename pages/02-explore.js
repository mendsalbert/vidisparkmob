...
const Explore = ({ catagoryfilter, data, filter, singleCatagoryfilter }) => {
  ...
  useEffect(() => {
    setFilterData(data && simpleProductFilter(catagoryFilter, data));
  }, [data, catagoryFilter]);

  const filterCatagory_ = (value) => {
    setCatagoryFilter(value.toLowerCase());
    setFilter_({ ...filter_, category: [value.toLowerCase()] });
    setFilterData(data && simpleProductFilter(value, data));
  };
  ...
  const onSubmit = (e) => {
    ...
    setFilter_({
      ...filter_,
      category: catagoryFilter.length > 0 ? [catagoryFilter] : [],
    });
    setFilterData(data && getProductByFilter(data, filter_));
    setActive(0);
  };
